	...
	public class ShapeMeshCache
	{
		...
		void SetStrokeRenderKeywords(Material mat, StrokeRenderType renderType)
		{
			...
			for (int i = 0; i < allRenderTypes.Length; i++)
			{
				...
				{
					if (keyword == GetRenderTypeKeyword(allRenderTypes[i]))
					{
				...
				{
					if (allRenderTypes[i] == renderType)
		...
		void SetCornerRenderKeywords(Material mat, StrokeCornerType cornerType)
		{
			...
			for (int i = 0; i < allCornerTypes.Length; i++)
			{
				...
				foreach(var keyword in keywords)
                {
					if (keyword == GetCornerTypeKeyword(allCornerTypes[i]))
					{
						localKeyword = keyword;
						break;
					}
				...
				if (localKeyword.Length > 0)
				{
					if (allCornerTypes[i] == cornerType)
						mat.EnableKeyword(localKeyword);
					else
						mat.DisableKeyword(localKeyword);
				}
